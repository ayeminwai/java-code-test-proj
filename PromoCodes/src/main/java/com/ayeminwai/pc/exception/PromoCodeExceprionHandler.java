...
import com.ayeminwai.pc.beans.res.CoreErrorRes;
...
@ControllerAdvice
@Log4j2
public class PromoCodeExceprionHandler extends ResponseEntityExceptionHandler {
	...
	public final ResponseEntity<Object> handleCacisException(PromoCodeException cacisEx, WebRequest request) {
	...
	protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException ex, HttpHeaders headers, HttpStatus status, WebRequest request) {
		...
		PromoCodeException cacisEx = new PromoCodeException(HttpStatus.BAD_REQUEST, IMessages.IError.VALIDATION_FAILED, APICodes.VALIDATION_ERR, errors);
	...
	private HashMap getErrorMap(Object ex) {
		...
		if(ex instanceof PromoCodeException){

			objCoreErrorRes = new CoreErrorRes();
			objCoreErrorRes.getCoreErrorResponse((PromoCodeException)ex);

			if(((PromoCodeException) ex).getE() != null) {
				
				errorDetail = ((PromoCodeException) ex).getE().getStackTrace()[0] + "-" + ((PromoCodeException) ex).getE().getMessage();
